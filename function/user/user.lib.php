...
function register(string $username, string $nickname, string $password, string $repeat)
{
    $acctable = \DB::tname('account');
    $resultdata = [false, ''];

    if (!CheckUsernameFormat($username) || !CheckPasswordFormat($password) || $password != $repeat ||
    ...
    if (!\DB::queryEx("INSERT INTO `$acctable` ".
                    '(`uid`, `username`, `passhash`, `nickname`, `timestamp`) '.
                    'VALUES (NULL,?,?,?,CURRENT_TIMESTAMP)', $username, $password, $nickname)) {
        $resultdata[1] = '帳號已被註冊';
...
function login(string $username, string $password)
{
    ...
    if (CheckUsernameFormat($username)) {
        $res = \DB::fetchEx("SELECT `username` FROM `$acctable` WHERE `username`=?", $username);
        if ($res === false) {
            $resultdata[1] = '無此帳號';
